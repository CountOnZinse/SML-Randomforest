...
"randomForest.default" <-
  function(x, y=NULL,  xtest=NULL, ytest=NULL, ntree=500, # checked
           mtry=if (!is.null(y) && !is.factor(y))
             max(floor(ncol(x)/3), 1) else floor(sqrt(ncol(x))), # checked
           weights=NULL,
           replace=TRUE, classwt=NULL, cutoff, strata,
           sampsize = if (replace) nrow(x) else ceiling(.632*nrow(x)), # check
