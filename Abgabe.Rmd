...
One of the main advantages of classification trees is their interpretability. The visual structure of a tree is easy to understand, making it possible to explain the decision-making process to non-technical stakeholders. Furthermore, classification trees can handle both numerical and categorical data without requiring much pre-processing, such as normalization or scaling. However, classification trees are not without drawbacks. They are prone to overfitting, especially when the tree grows too deep, capturing noise rather than the underlying pattern. Techniques like pruning, where parts of the tree are removed after the initial training, can help mitigate this problem. Pruning in classification trees is a crucial step to balance the complexity of the model with its ability to generalize well to new data, making the model more robust and interpretable.
...
The final data set is generated with `p = 36` continuous predictor variables, `n = 20,000` observations, and correlations between `min_cor = -0.3` and `max_cor = 0.5`. While the function creates two outcome variables, the multinomial variable representing cover tree types and a binary variable indicating tree health, only the multinomial variable will be used for classification, as computational limits in this analysis only allow the classification of a single variable. The distribution of the multinomial dependent variable `y` is shown below.

In the left chart of the figure below, it is visible that the distribution is more balanced than the empirical data, with four out of the seven classes making up the majority of all.
...
To evaluate the performance of the random forest implementations, the following Goodness-of-Fit (GoF) measures are used:
...
The R package random forest generator (known as "*ranger*") is an implementation for random forests with optimizations for high dimensional data in R [@schwarz2010]. The developers claim to provide a package which is comparably simple to use [@wright2017, p. 2].

The package is optimized by extensive memory profiling for different types of input data. As a random forest is proceeded in R, all values of all features of `mtry` are splitting candidates. Since the required computational power increases exponentially with added features, this is the bottleneck for many random forests packages and the developers of the ranger package focused on this problem.
...
For the cross-validation, a function `cv_rf` is created to apply classification by the random forests algorithm, while drawing on the functions implemented in the three selected R packages: `randomForest`, `ranger`, and `cforest`. With `cv_rf`, the models' performance based on Accuracy, F1 score, and F-\$\\beta\$ score is estimated, while also recording the computational time taken to fit each model.
...
The cross-validation is targeted at obtaining the optimal hyperparameter combinations as measured in different GoF indicators for each type of data, empirical as well as simulated data,.
...
The analysis will focus on the output of the algorithm implemented in the `cv_rf` function with respect to each of the three tested R packages, *randomForest*, *ranger* and *party*. At first sight, the packages show different patterns regarding the required computational time and in the GoF measures. Each step is equivalent to a different hyperparameter combination as is visible in the table below. Values of `ntree` range from $450$ to $650$, while values from `mtry` range from $3$ to $7$.

| Step | Value of ntree | Value of mtry |
|------|----------------|---------------|
| 1    | 450            | 3             |
| 2    | 450            | 4             |
| 3    | 450            | 5             |
...
In case of the GoF measure accuracy, as can be seen in the two figures below, the R packages *randomForest* and *ranger* are generating higher values than the *party* package for both simulated and empirical data. This is especially true for the empirical data, for which the *party* package is less efficient than *randomForest* and *ranger*. Regarding the slope, the pattern seems to be similar. For all three packages, there is a higher accuracy when performing classification on the empirical data than on simulated data.

![**Simulated data**](pics/GoF_Accuracy.png){width="60%"}

![**Empirical data**](pics/GoF_emp_Accuracy.png){width="60%"}

In the two figures below, the F1-Scores across the different hyperparameter combinations are shown. The same assumption regarding the accuracy applies to the F1 score. For the combination of the arguments, `ntree = 550` and `mtry = 7`, the package *randomForest* achieves the highest overall score compared to the other packages -- this yields for the simulated data. The package *ranger* achieve the highest F1-Scores for the classification of the empirical data.

![**Simulated data**](pics/GoF_F1%20Score.png){width="60%"}

![**Empirical data**](pics/GoF_emp_F1%20Score.png){width="60%"}

The three packages show similar results for the F-$\beta$-Score. The *party* package achieves the highest overall score compared to the other packages for the hyperparameter combination of `ntree = 550` and `mtry = 6` -- this is true for the simulated data. Regarding the classification of the empirical data, *ranger* performs best. This could be seen as a hint that those packages perform better in case of precision.

![**Simulated data**](pics/GoF_F-beta%20Score.png){width="60%"}

![**Empirical data**](pics/GoF_emp_F-beta%20Score.png){width="60%"}

Apart from the GoF measures, the analysis also includes measures on the computational time required to perform classification with the random forests implementations. As is visible in the two figures below, the RF implementation in the *party* package requires less computational time than the *randomForest* and *ranger* package. *Ranger* has, in general, the most time-consuming implementation. Regarding the classification on empirical data, the implementation in the *party* package is less time consuming compared to the *randomForest* package.

![**Simulated data**](pics/GoF_Time.png){width="60%"}

![**Empirical data**](pics/GoF_emp_Time.png){width="60%"}

One of the reasons for the differing results of the packages with respect to computational time and the GoF could be those hyperparameters in the functions which were not tested in the analysis. For this reason, the analysis included a further investigation of how the random forest algorithm was implemented in the different packages. The source code can be accessed via CRAN for each individual package. All of the three analyzed packages refer to other programming languages for the computation of the random forest algorithm itself.

In case of the *randomForest* package, the computation of the random forests algorithm is written in Fortran, with an R port to provide access to the implementation in R [@breiman2002]. The *ranger* package refers to C++ [@wright2017] while in the case of the *party* package, it is not clarified how the algorithm is implemented apart from the R function [@hothorn2024; @hothorn2024a]. For the final implementation in the R functions, each package includes helper functions checking and adjusting the hyperparameters. The manual specification via specifying the arguments of the packages' functions serves as an input for the function in the background, which computes the desired output.

It can be seen that the default argument in *randomForest*, *ranger* and *party* is the same for `ntree = 500` (underline in green in the imagine beneath). Also, the number of variables used for each split for the selection `mtry` can be specified. Comparing the R packages, it can be seen that `mtry = floor(sqrt(ncol(x)))` for *randomForest*, which adjusts downward and `mtry = ceiling(sqrt(nvar))` for party, which adjusts upward (underline in orange in the imagine beneath). The *ranger* package does not adjust this hyperparameter in the R source code, therefore, the expectation is an implementation in the C++ script. Nonetheless, the documentation of the ranger package states $\sqrt p$ as the default argument.
...
Interesting to note is the differing performance between varying specifications of `n = 20,000`, which were specified for all of the figures above, and `n = 10,000`. This is especially true for the simulated data. Comparing the figure below with the first figure on the F-$\beta$ Score for empirical data, there are large differences in the F-$\beta$ Score for the *party* package which seems to perform extensively better with a higher `n`.

![**Empirical data**](pics_n10000/GoF_F-beta%20Score.png){width="60%"}

# Conclusion

To sum up – the following recommendation is made. As for random forests beginners, it is reasonable to start with basic packages which provide good documentation and well explained hyperparameters. For this reason, this paper recommends using the R package *randomForest* itself. The *randomForest* package is a direct product of Leo Breiman, which could be used as useful link between theory and practice.

For users familiar with the random forests algorithm, it is recommended to use the *ranger* package. *Ranger* provides a more detailed documentation and allows for specifying more hyperparameters. As the *ranger* package is optimized for high-dimensional data and includes parallelization techniques, this aspect presents itself as starting points for future research. The performance could be improved by increasing the number of threads used while the default are the available CPUs.

Lastly, the *party* package offer a special kind of the random forests algorithm as the function `cforest` provides an alternative with conditional random forests. The documentation of the package and especially for the `cforest` function can be improved compared to the extensive documentation of the other two packages included in the analysis of this paper. Similar to the *ranger* package, the *party* package draws on parallelization techniques without further specifying the actual implementation. As this package is comparably new, further changes in the implementation can be expected.

The *party* package can also be seen as a package example of a random forests implementation build from scratch. For users interested in building own functions or own implementations for the random forest algorithm, the *party* package could be a potential candidate.

To further trace back and understand the implementation of the random forest algorithm in C++ or Fortran, additional knowledge and advanced programming skills are needed, as well as more extensive documentation of the implementations.

With additional computational capacities, it could be valuable to further study the behaviours of the packages regarding additional changes in the sample size `n`. In this paper, major differences between packages and also within the same package were discovered. This realisation indicates that many possible hyperparameters and functional arguments that can be specified also shows that the remaining question - which packages are more efficient - can be further investigated.
